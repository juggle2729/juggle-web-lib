<!--多文件上传、新UI规范的bbutton上传的upload组件 -->
<template>
  <div>
    <section :style="{ 'max-width': width }">
      ...
      >
        <el-button size="small" type="primary" :disabled="Boolean(fileList.length == limit)" @click="uplodad"
          ><i class="el-icon-folder-add" style="margin-right: 3px"></i>
          <span class="text">
             <slot></slot>
          </span>
        </el-button>
      ...
      <div v-for="(item, index) in fileObjectList" :key="item.uuid" class="uploadBoxList">
        ...
        >
          ...
          <div class="uloadContent">
            ...
            <div class="uploadPengding commonBox" v-if="item.percentage && item.percentage != 100">
              {{ item.percentage }}%
            </div>
  ...
  methods: {
    init(array) {
      ...
      array.forEach(element => {
        ...
        this.fileList.push(element.externalUrl);
        this.fileObjectList.push(element);
        this.snList.push(element.sn);
      });
      if (this.fileObjectList.length > 0) {
        this.processIndex = this.snList.length;
        this.successOrEoorIndex = this.snList.length;
      }
